...
| `GetDebugState()` | Returns a summary only when debug mode is enabled; includes `CommandCount`, `SearchItemCount`, and `HasOpenCommandPalette`. |
...
Sliders support decimal `Min`, `Max`, and `Increment` values. Display precision follows the configured increment/range precision, and drag updates remain snapped and clamped.

Dropdowns accept `Searchable = true|false` (or `Search`) and `SearchThreshold`. When no explicit search setting is supplied, lists with at least eight options add a filter input automatically. Search preserves existing flag/controller/config behavior.
...
The palette is beneath dialogs and above windows; opening a dialog closes the palette. `Ctrl+K` opens/toggles the palette unless a normal text input is focused. Palette/dropdown results use arrow keys and `Enter`; `Escape` closes the active palette, dropdown, or dialog.
